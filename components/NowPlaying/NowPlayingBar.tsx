...
export function NowPlayingBar() {
  ...
  const { currentTrack, isPlaying, seek, duration, seekTo, audioRef } = useAudioPlayer();
  ...
  return (
    <>
      <Modal.Root opened={opened} onClose={close} fullScreen radius={0} transitionProps={{ transition: 'fade', duration: 200 }}>
        <Modal.Overlay />
        <Modal.Content>
          <Modal.Header>
            ...
            {opened && (
              <Center>
                <Stack align="center" gap="4" w="100%" px="md" py="sm">
                  <Image src={currentTrack?.image || musicPlaceholder.src} w={350} h={350} radius="md" fit="cover" />
                  <Text fw={700} size="32px" ta={'center'}>
                    {currentTrack?.title}
                  </Text>

                  <Text ta="center" c="dimmed" size="md" truncate w="100%">
                    {currentTrack?.subtitle ? `${currentTrack.subtitle} • ${currentTrack.year}` : `${currentTrack.artist} • ${currentTrack.genre} • ${currentTrack.year}`}
                  </Text>
                  ...
                  <Group justify="center" mt="md" gap="md">
                    ...
                    <ActionIcon variant="light" size="4rem" radius="lg" onClick={() => dispatch(isPlaying ? pause() : play())}>
                      {isPlaying ? <IconPlayerPauseFilled size="3.6rem" stroke={1.5} /> : <IconPlayerPlayFilled size="3.6rem" stroke={1.5} />}
                    </ActionIcon>
                  ...
                  <Drawer
                    ...
                    padding="xs"
                    radius="md"
                    styles={{ body: { paddingTop: rem(12) } }}
                  >
                    <Center>
                      <Box style={{ width: rem(40), height: rem(4), borderRadius: rem(4), backgroundColor: '#ccc', marginBottom: rem(12) }} />
                    </Center>

                    <Stack>
                      {playlist.length > 0 ? <PlaylistDndList /> : <SkeletonSongBar count={8} />}
                    </Stack>
                  </Drawer>
                </Stack>
              </Center>
      ...
      <Card radius={0} p={0} w="100%" onClick={open}>
        ...
        <Flex justify="space-between" align="center" p={'xs'} style={{ flex: 1, minWidth: 0 }}>
          <Group wrap="nowrap" style={{ flex: 1, minWidth: 0 }}>
            <Image src={currentTrack?.image || musicPlaceholder.src} radius="md" w={48} h={48} alt={currentTrack?.title} />
            <Box style={{ minWidth: 0 }} pr={10}>
              <Text size="md" fw={500} truncate>{currentTrack?.title ?? 'No track playing'}</Text>
          ...
          <Group gap="sm" wrap="nowrap">
            <ActionIcon variant="subtle" px={0} size="xl" radius="lg" onClick={(e) => doNoExpandAndPlay(e)}>
